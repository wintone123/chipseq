...
# main
print("+++++++++loop start+++++++++")
for(num in c(1:length(chrom_list))){
    ...
    for(file in peak_file_list){
        ...
        if(name_split[4] == paste0(chrom_name, ".narrowPeak")){
            assign(paste0(name_split[1], "_peak"), import_np(file.path(path, "narrowpeak_split_files", file)))
            peak_list <- append(peak_list, paste0(name_split[1], "_peak"))
        }
    }
    for(i in c(2:length(peak_list))){
        if(i <= 2){
            overlap_peak <- overlap_peaks(eval(parse(text = peak_list[1])), eval(parse(text = peak_list[2])))
        } else{
            overlap_peak <- overlap_peaks(overlap_peak, eval(parse(text = peak_list[i])))
        }
    }
    ...
    for(i in c(1:nrow(gene_info_fil))){
    ...
    for(i in c(1:nrow(gene_info_fil))){
        if(as.logical(bin_test@strand[(i-1)*150+1] == "+")){
            sample_hits <- bin_test$score[c(((i-1)*150+1):(i*150))]
        } else{
            sample_hits <- rev(bin_test$score[c(((i-1)*150+1):(i*150))])
        }
    ...
    for(i in c(1:nrow(gene_info_fil))){
        if(as.logical(bin_control@strand[(i-1)*150+1] == "+")){
            control_hits <- bin_control$score[c(((i-1)*150+1):(i*150))]
        } else{
            control_hits <- rev(bin_control$score[c(((i-1)*150+1):(i*150))])
        }
