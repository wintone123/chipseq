...
for(gene_num in c(1:nrow(gene_df))){
  ...
  if(gene_num == 1){
    ...
    for(file_name in load_list){
      ...
      if(length(file_split) == 3 & file_split[3] == paste0("chr", chrom, ".bed")){
        read_name <- file_split[1]
        assign(read_name, prepareChipseq(import.bed(file.path(path, file_name))))
        read_list <- c(read_list, read_name)
      }
      if(length(file_split) == 4 & file_split[4] == paste0("chr", chrom, ".narrowPeak")){
        peak_name <- paste0(file_split[1], "_peaks")
        assign(peak_name, import_np(file.path(path, file_name)))
